...
for i in tqdm(range(bootstrapping), desc='Processing'):
    boot = resample(ids, replace=False, n_samples=round(len(ids) * sample_size), random_state=i)
    s = sub[boot]
    yi = y[boot]

    clusters_sample = clusters[:, boot]
    func_m = partial(worker_m, clusters_sample)
    with Pool(multiprocessing.cpu_count()) as p:
        m = list(p.map(func_m, [i for i in range(clusters_sample.shape[1])]))
...
plt.savefig('sensitivity1.png', dpi=300)
plt.show()
